...
void create_script(std::string filename,
    ...
    script << "### Create necessary variables\n"
           ...
           << "\nisoValue=" << std::to_string(isoValue)
           << "\nnumCellsInLargestAxis=" << std::to_string(numCellsInLargestAxis) << "\n\n";
:set nonu                                                                                                                                                       1,1           Top
                   std::string neighborhoodSearch, std::string kernelFunction, std::string levelFunction, std::string reconstructionMethod,
                   std::string inputFolder, std::string outputFolder,
                   float smoothingLength, float compactSupport, float isoValue, int numCellsInLargestAxis,
    ...
    script << "### Execute and time the application\ntime ./surfaceReconstruction.exe "
           ...
           << "$inputFolder $outputFolder $smoothingLength $compactSupport $isoValue $numCellsInLargestAxis";
}
